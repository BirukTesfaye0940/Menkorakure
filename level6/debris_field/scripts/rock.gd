...
func _ready() -> void:
	$Sprite2D.texture = preload("res://level6/debris_field/assets/rocks/rock.png")
	gravity_scale = 0
	freeze = true
...
func destroy() -> void:
	if current_scale * split_scale_factor > min_size:
		...
		var explosion: Node2D = preload("res://level6/effects/scenes/explosion.tscn").instantiate()
...
func split() -> void:
	for i in range(2):
		var small_rock: RigidBody2D = load("res://level6/debris_field/scenes/rock.tscn").instantiate()
