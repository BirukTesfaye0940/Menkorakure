...
var rock_scene: PackedScene = preload("res://level3/Scene/rock.tscn")  # Use preload for better performance
...
func _on_rock_collision(rock_node):  # Receive the rock node
	...
	if Global.healthT <= 0:
		...
		get_tree().change_scene_to_file("res://level3/Scene/game_over.tscn")
