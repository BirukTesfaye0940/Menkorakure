...
func interact():
	...
	# Load Gameplay 2 (maze room) scene
	await get_tree().create_timer(1.0).timeout  # small delay before transition
	var maze_scene = preload("res://Level1/scenes/game.tscn").instantiate()
	get_tree().root.add_child(maze_scene)
	get_tree().current_scene.queue_free()
	get_tree().current_scene = maze_scene
