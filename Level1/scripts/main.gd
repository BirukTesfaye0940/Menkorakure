...
@onready var puzzle_solved_ui = $puzzleSolved
...
func _ready():
	...
	timer_overlay.connect("timer_completed", _on_timer_completed) ####
...
# Hide the full puzzle image to prevent it from being displayed prematurely
	$FullImage.hide()
	for tile in tiles:
		tile.hide()
# calles the visible method to display the Ui 
...
func start_game():
	...
	var grayimage=Image.load_from_file("res://Level1/newAssets/empty.jpg")
...
func _process(delta):
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and mouse:
		...
		if tiles == solved:
			...
			$FullImage.show()
			# Hide all puzzle tiles
			for tile in tiles:
				tile.hide()
				puzzle_solved_ui.visible = true
				$FullImage.hide()
				get_tree().paused = true  
			
# Function to randomly shuffle the tiles at the start of the game
func shuffle_tiles():
	t=0
	 # Loop to perform 30 random tile movements to create a shuffled state
	while t<3:
		var random_tile=randi() %16
		# Ensure the selected tile is not the empty tile (`tile16`) and isn't the previous tile moved
	...
	movecounter=0
	 # Reset empty tile index to the last position
	empty_tile_index=15 
	 # Clear `previous` to allow fresh tracking of tile movements
	previous=""
